...
const MDNS_BROWSE_BUF_SZ: usize =
    MDNS_BROWSE_LABEL_SZ + MDNS_BROWSE_HOST_SZ + MDNS_BROWSE_TXT_SZ;
...
impl<'a, 'd> OtMdns<'a, 'd> {
    ...
    async fn run_resolve(
        ...
        loop {
            ...
            // NOTE: `rs-matter` uses `heapless` 0.8, so this buffer must be the
            // 0.8 `String` (aliased here as `heapless_08`).
            let mut instance_name = heapless_08::String::<128>::new();
            ...
            let result = ot
                .dns_resolve_service_and_host_address(
                    ...
                    |response| {
                        ...
                        matter.transport().try_deposit_mdns_resolve(&MdnsRemoteService {
    ...
    async fn run_browse(
        ...
        loop {
            ...
            // (`rs-matter` uses `heapless` 0.8 - see the resolve loop note.)
            let mut service_type = heapless_08::String::<64>::new();
            ...
            let result = ot
                .dns_browse(service_name.as_str(), None, |response| {
                    ...
                    loop {
                        ...
                        let Ok(info) =
                            response.service_info(label, &mut *host_buf, &mut *txt_buf)
                        else {
                            continue;
                        };
                        ...
                        matter.transport().try_deposit_mdns_browse(&MdnsRemoteService {
                            instance_name: DottedName(instance_name.as_str()),
                            port: Some(info.port),
                            addrs: core::iter::once(IpAddr::V6(addr)),
                            txt: TxtEntries::new(info.txt_data.unwrap_or(&[])),
