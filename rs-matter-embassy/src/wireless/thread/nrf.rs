...
// Stubs for HCI commands that `trouble-host` *references* but that this
// peripheral-only build can never *reach* - the peripheral-only SoftDevice
// Controller library (see `nrf-sdc` in `Cargo.toml`; legacy advertising only,
// no scanner, no central role) does not provide them, and without the stubs
// the references fail the link:
//
// - the scanner and central-connect commands: `trouble-host`'s run loop
//   handles scan/connect *cancellation* without a feature gate; only the code
//   that could ever start those operations is `scan`/`central`-gated (and
//   compiled out here);
// - the extended-advertising enable: referenced by the runner's
//   advertising-cancellation handling for the extended case; this crate's BTP
//   advertising is legacy-only (`Advertisement::ConnectableScannableUndirected`),
//   so the extended branch never runs. If extended advertising were ever
//   requested, the stub would surface as a loud advertising failure, not
//   silent misbehavior.
//
// Remove these (and re-add the `central` features to `trouble-host`/`nrf-sdc`)
// if the BLE central role is ever needed - they would then be duplicate
// symbols.
macro_rules! sdc_hci_cmd_stub {
    ($($name:ident),* $(,)?) => {
        $(
            #[no_mangle]
            extern "C" fn $name(_p_params: *const core::ffi::c_void) -> u8 {
                // "Unknown HCI Command"
                0x01
            }
        )*
