...
use rs_matter_stack::wireless::GattPeripheral;
    ...
    C: Controller,
{
    ...
    async fn run_peripheral(
        ...
        loop {
            ...
            let subscribed = Self::process_subscribe(server, &conn)
                .await
                .map_err(to_matter_err)?;
    ...
    /// Stream events until the other peer subscribes to char C2
    async fn process_subscribe(
        server: &Server<'_>,
        conn: &GattConnection<'_, '_, DefaultPacketPool>,
    ) -> Result<bool, trouble_host::Error> {
                ...
                GattConnectionEvent::Gatt { event } => {
                    ...
                    match event.payload().incoming() {
                        AttClient::Request(AttReq::Write {
                            ...
                            if Some(handle) == server.matter_service.c2.cccd_handle {
